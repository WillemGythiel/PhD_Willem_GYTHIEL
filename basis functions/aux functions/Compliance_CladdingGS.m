function [fC] = Compliance_CladdingGS(Nodes,Trigs,Pdis,varargin)
if nargin < 4
...
function [c] = CompCladding(Nodes,Trigs,Load,varargin)
E =  90e06;   nu = 0.3;  h = 0.02;  prp = [h E nu];
info = SubTriginfoGS(5);
...
for i = 1:nTri
    ...
    elseif strcmp(varargin{1},'shape')
       ...
       dC = @(d) U'*(-dK(d)*U+2*dP(d));
       
       out = arrayfun(@(y) dC(y),(1:9));
    end  
  c(i,ind) = c(i,ind)+out;
end
end
