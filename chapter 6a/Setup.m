...
Cen = NdID(abs(sum(Node(1:end-1,2:3),2))<1e-5);
Problem.DID = Dia;  % Diagonal nodes
Problem.int = int;
if any(strcmp(gradient,{'shape','both'}))
    PrX = NdID(Per(2)&~Per(3));
    PrY = NdID(Per(3)&~Per(2));
    Problem.CID = [Addkron(int,[1;2;3]*num);PrY+1*num;PrY+3*num;PrX+2*num;Dia+1*num;Dia+3*num;Cen+3*num];  % Indices of xVars in Nodes
    Problem.SID = [Addkron(3*(int-1),[1;2;3]);3*(PrY-1)+1;3*(PrY-1)+3;3*(PrX-1)+2;3*(Dia-1)+1;3*(Dia-1)+3;3*(Cen-1)+3]; % Indices of xVars in xAll list
    ...
    for desVar = 1:size(xvalC,1)
        if mod(Problem.SID(desVar),3)==0
            xminC(desVar) = max(xminC(desVar),xvalC(desVar)-3);
            xmaxC(desVar) = min(xmaxC(desVar),xvalC(desVar)+3);
        else
